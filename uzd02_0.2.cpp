...
void failoived(vector<studentas> &A)
{
    ...
    while (getline(buffer, eil))
    {
        ...
        //eilvec.push_back(eil);
    }
    /* while(buffer){
        if(!buffer.eof()){
            A.push_back(studentas());
            buffer >>  A[i].vard >> A[i].pav;
            int num;
            while (buffer >> num)
            {
                A[i].nd.push_back(num);
    ...
    std::cout << "Eilučių vektoriiaus konvertavimas į vieną eilutę užtruko: " << diff.count() << " s\n";
    cout << i << endl;
    /*for (int i = 0; i < A.size(); i++)
    {
        output += A[i].vard + " " + A[i].pav + " ";
        for (int j = 0; j < A[i].nd.size(); j++)
        {
            string s = to_string(A[i].nd[j]);
            output += s + " ";
        }
        output += "\n";
    }

    ofstream out_f("test.txt");
    out_f << output;
    out_f.close();*/
}
void duomenys(int &studsk, int &n, vector<studentas> &A)
...
void rez( vector<studentas> &A)
{
    ...
    /*if (answ == 'y')
        ...
        {
            cout << A[i].vard << " "
                 << " " << A[i].pav << " " << A[i].vidurkis * 0.4 + 0.6 * A[i].egz << " " << A[i].median << endl;
